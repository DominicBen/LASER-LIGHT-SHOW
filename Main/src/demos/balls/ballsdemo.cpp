...
void BallDemo::init()
{
    ...
    for (size_t i = 0; i < 7; i++)
    {
        ...
        GameObject2D *ball = (new GameObject2D(ballg))->setPosition({WIDTH / 2 + offset, HEIGHT / 2 + offset})->setScale({100 * (coolrandomcolor + 1) / 2, 100 * (coolrandomcolor + 1) / 2})->setColor((Color)i)->fitColliderToObject();
