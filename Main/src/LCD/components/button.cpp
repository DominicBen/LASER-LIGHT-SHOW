...
Button::Button(String text_, Callback callback_, int callback_value_) : Component(text_), mCallback(callback_), mCallbackValue(callback_value_)
{
}
...
void Button::draw()
{
    ...
    tft.setTextSize(1);
    tft.textEnlarge(1);
    tft.textSetCursor(pos.x - scale.x / 4, pos.y - scale.y / 4 - 4);
    tft.print(text);
}
