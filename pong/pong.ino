...
void loop() {
if(gameEnd==false){  
    if(ballY >= 234){//bottom bounce
      ballSpeedY *= -1;
      playFile("bounce.WAV");
      delay(25);
    }
    if(ballY <= 5){//top bounce
      ballSpeedY *= -1;
      playFile("bounce.WAV");
      delay(25);
    }
    if(ballX >= rightPaddleX-4 && ballY >= rightPaddleY && ballY <= rightPaddleY+30){//right paddle bounce
      ballSpeedX *= -1;
      playFile("bounce.WAV");
      delay(25);
    }
    if(ballX <= leftPaddleX+5 && ballY >= leftPaddleY && ballY <= leftPaddleY+30){//left paddle bounce
      ballSpeedX *= -1;
      playFile("bounce.WAV");
      delay(25);
    }
    if(ballX >= 315){//right goal
      player1++;
      playFile("goal.WAV");
      delay(25);
      resetRound();
    }
    if(ballX <= 5){//left goal
      player2++;
      playFile("goal.WAV");
      delay(25);
      resetRound();
